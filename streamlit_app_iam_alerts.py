...
def load_alerts():
    all_alerts = []
    pattern = "iam_alerts_output_1/run_20260114_215343/*_alerts.json"
    ...
    if st.session_state.results:
        ...
        with col1:
            if st.button("💾 Save JSON Report"):
                ...
                filename = f"iam_batch_analysis_{timestamp}.json"
                export_data = {
                    "metadata": {
...
"""
        ...
        with col3:
            if st.button("📊 Export KQL Queries"):
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"iam_dynamic_kql_queries_{timestamp}.sql"
